...
void HLEDMAT_voidDisplay(u8 *Copy_u8Date,u8 Copy_u8SizeArr)
{
	while(1)
	{
	for(int j=0 ;j<(Copy_u8SizeArr-8);j++)
	{
	for(int i=0 ;i<200;i++)
	{
	/*  Colum 0 */	
	DisableAllCols();
	SetRowValues(Copy_u8Date[0+j]);
	...
	/*  Colum 6 */	
	DisableAllCols();
	SetRowValues(Copy_u8Date[6+j]);
	MGPIO_voidSetPinValue(LEDMAT_COL6,GPIO_LOW);
	/* Start timer */
	MSTK_voidSetBusyWait(2500); //2.5 MSEC
	/*  Colum 7 */	
	DisableAllCols();
	SetRowValues(Copy_u8Date[7+j]);
